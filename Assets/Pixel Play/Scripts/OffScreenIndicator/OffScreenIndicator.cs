...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void DrawIndicators()
    {
        foreach(Target target in targets)
        {
            ...
            target.IsVisible = isTargetVisible;
            float distanceFromCar = target.NeedDistanceText ? target.GetDistanceFromCamera(GameController.Instance.Car.transform.position) : float.MinValue;// Gets the target distance from the camera.
            ...
            else if(target.NeedArrowIndicator && !isTargetVisible)
            {
                ...
                indicator.Arrow.transform.rotation = Quaternion.Euler(0, 0, angle * Mathf.Rad2Deg); // Sets the rotation for the arrow indicator.
              //  indicator.SetImag(target.StartImage);// Sets the image  of the indicator.
                indicator.SetImageColor(Color.white);// Sets the image color of the indicator.

            }
            if (indicator)
            {
                target.BelongedIndicator = indicator;
                indicator.BelongedPickup = target.gameObject;
                indicator.SetDistanceText(distanceFromCar); //Set the distance text for the indicator.
